...
export function Spotlight({ className, size = 200, springOptions = { bounce: 0 } }: SpotlightProps) {
  ...
  useEffect(() => {
    ...
    // Upstream declares these inline in BOTH addEventListener and removeEventListener,
    // which creates two different function objects — so the enter/leave listeners are
    // never actually removed and leak on every unmount. Named handlers fix it.
